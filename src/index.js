...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  return response.status(403).json({ error: "usuario invalido" });
}
...
function checksTodoExists(request, response, next) {
  ...
  if (!validId) return response.status(400).json({ error: "id invalido" });

  const findUser = users.find((u) => u.username === username);
  if (!findUser)
    return response.status(404).json({ error: "usuario nao encontrado" });

  const findTodo = findUser.todos.find((t) => t.id === id);
  if (!findTodo)
